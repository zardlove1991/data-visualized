  ...
  <div class="maanshan-news" id="maanshan-news">
    <div class="news-wrap sys-flex sys-vertical" >
      ...
      >
        ...
        >{{k+1}}</div> -->
        <div class="list-icon" v-if='!(k == 0 && page == 2)'>
          {{ v.type == 'article' ? '[ 文稿 ]' : v.type == 'video' ? '[ 视频 ]' : '' }} 
        </div>
         <span class="new-icon" v-if='k == 0 && page == 2'>
        </span>
        ...
        <div class="list-number sys-flex sys-flex-center flex-justify-center">
          ...
          <span class="list-number-text">{{ v.click_num * 13 }}</span>
        </div>
        <div class="list-number sys-flex sys-flex-center flex-justify-center">
          <span class="list-number-icon time-icon"></span>
          <span class="list-number-text">{{ v.publish_time.slice(5, 16) }}</span>
        </div>
        <!-- <div class="list-title overhidden">{{v.title}}</div>
...
export default {
  ...
  mounted () {
    ...
    console.log(this.setFontsize('maanshan-news'))
    setInterval(() => {
      this.getDataList()
    }, this.frequency)
  },
  ...
  .news-list {
    ...
    .list-number {
      ...
      .list-number-icon {
        ...
        margin-right: 0.35em;
        &.read-icon {
          background: url('./assets/reader.png') no-repeat center;
          background-size: 100% 100%;
        }
        &.time-icon {
