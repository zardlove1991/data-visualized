  ...
  <div class="common01-cluegather-other">
    <div class="cluegather-wrap common01-border">
      ...
      <div class="wrap-content">
        <div class="list-sort">
            <img class="imgHeight" :src='require("./assets/zixun.png")'  alt="">
        </div>
      ...
      <div class="wrap-content">
        <div class="list-sort">
            <img class="imgHeight" :src='require("./assets/weixin.png")'  alt="">
        </div>
        <div class="item-list sys-flex sys-flex-center flex-justify-between animated" v-for="(v, k) in dataList02" :key="k" :class="{'flipInX' : v.title, 'marginBottom75': customSize}" :style="{'animation-delay' : k/2+'s'}">
          <div v-if="k == 0" class="common01-ft32 idx1">{{k+1}}</div>
          <div v-else-if="k == 1" class="common01-ft32 idx2">{{k+1}}</div>
          <div v-else-if="k == 2" class="common01-ft32 idx3">{{k+1}}</div>
          <div v-else class="common01-ft32 idx4">{{k+1}}</div>
      ...
      <div class="wrap-content">
        ...
        <div class="item-list sys-flex sys-flex-center flex-justify-between animated" v-for="(v, k) in dataList03" :key="k" :class="{'flipInX' : v.title, 'marginBottom75': customSize}" :style="{'animation-delay' : k/2+'s'}">
          <div v-if="k == 0" class="common01-ft32 idx1">{{k+1}}</div>
          <div v-else-if="k == 1" class="common01-ft32 idx2">{{k+1}}</div>
          <div v-else-if="k == 2" class="common01-ft32 idx3">{{k+1}}</div>
          <div v-else class="common01-ft32 idx4">{{k+1}}</div>
...
export default {
  ...
  created () {
    ...
    const type = ['website', 'weChat', 'weiBo']
    for (let item of type) {
      this.getHotsList(item)
    }
  },
  methods: {
    setFontSize (size) {
          ...
          if (res.data.result.data.length) {
            ...
            setTimeout(() => {
              if (type === 'website') {
                ...
                this.dataList02 = res.data.result.data
              } else if (type === 'weiBo') {
                this.dataList03 = res.data.result.data
              }
            }, 100)
            if (this.isPaging) {
              this.page += 1
